...
int main(int argc, char **argv)
{
  ...
  if (out)
  {
    ...
    // If no list of symbols, print everything sorted by address
    if (!list)
    {
      for (c = 0; c < numsymbols; ++c)
      {
        fprintf(out, "%s = $%04x\n", symbols[c], addresses[c]);
      }
    }
  ...
  else
  {
    ...
    while (fgets(buffer, 80, in))
    {
      ...
      // In Hessian / Steel Ranger code labels begin with uppercase; these can be ignored in pagecross analyze
      if (isupper(name[0]))
        nameok = 0;
      if (!nameok)
        continue;
